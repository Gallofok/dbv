"""
by wenchen
refrence from :https://www.youtube.com/watch?v=01sAkU_NvOY&t=11350s
"""
import cv2
import cv2 as cv
import imutils
...
class Handdetector():
    ...
    # this function used to find the hand.default is only detect one hand
    ...
    def findpos(self, img, handno=0, draw=True):
        ...
        if self.results.multi_hand_landmarks:
            ...
            for id, lm in enumerate(hand.landmark):
                h, w, c = img.shape #w,h c is the ratio
                cx, cy = int(lm.x * w), int(lm.y * h)
                lmlist.append([id, cx, cy])
                if draw:
                    cv.circle(img, (cx, cy), 5, (255, 0, 255), cv.FILLED)#draw the
        return lmlist
    ...
    def gesturedet(self, gelenke, img):
        ...
        if np.abs(gesture[1][1] - gesture[2][1]) <= 15:#when the horizen distance of between two finger tips less than 15 pixel then
            ...
            return 1
        if np.abs(gesture[1][0] - gesture[2][0]) <= 15:#when the vertical distance of between two finger tips less than 15 pixel then
