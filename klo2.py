...
def cropthematerial(material='images/videoplayback 9s.mp4'):
    vc = cv.VideoCapture(material)
    fourcc = cv.VideoWriter_fourcc(*'XVID')
    frame_width = int(vc.get(3))
    frame_height = int(vc.get(4))
    # fourcc = cv.VideoWriter_fourcc('M','J','P','G')
    out = cv.VideoWriter('material.mp4', fourcc, 20.0, (frame_width, frame_height))  # the output video
    while vc.isOpened():
        ret, frame = vc.read()
        rec = (287, 30, 566, 470)
...
def setmaterbg(background='kongfu.mp4', speed=10,save = False):
    ...
    if save:
        ...
        frame_height = int(vc.get(4))
        # fourcc = cv.VideoWriter_fourcc('M','J','P','G')
        out = cv.VideoWriter('insertforeground.mp4', fourcc, 20.0, (frame_width, frame_height))  # the output video
    while vc.isOpened():
        ret, frame = vc.read()
        rowgrenz, colgrenz, _ = frame.shape
        if vd.isOpened():
...
def applyGraffiti(img, graffiti, pts_corner=(300, 300), totalcover=True):
    ...
    mask = gwarped.copy()
    mask[mask != (0, 0, 0)] = 1
    # multiply images (in the masked area) to get details of both
    if totalcover:
...
def facedet(input='images/do.mp4', fliter='haarcascades/haarcascade_frontalface_default.xml', widt=960, hei=540,
            speed=10):
    output = 'images/face'
    timeF = 5
    c = 1
...
# crop a single image
def croptheface(path="images/test.jpg", fliter='haarcascades/haarcascade_frontalface_default.xml'):
...
def realtimecolortransfer(source='images/do.mp4', target='images/sky-g29f24e740_640.jpg', widt=960, hei=540):
...
def facereplace(source='images/do.mp4', target='nn.jpg', widt=960, hei=540):
    fliter = 'haarcascades/haarcascade_frontalface_default.xml'
    vc = cv.VideoCapture(source)
...
def twovideocombination(source='kongfu.mp4', target='images/do.mp4', widt=960, hei=540, speed=10):
    vc = cv.VideoCapture(source)
